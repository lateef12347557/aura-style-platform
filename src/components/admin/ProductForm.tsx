  ...
  async function save() {
    ...
    try {
      const payload = {
        ...
        compare_price:
          form.compare_price === null || form.compare_price === undefined
            ? null
    ...
    <div className="space-y-8 max-w-4xl">
        ...
        <div className="grid grid-cols-3 gap-4">
          ...
          <Field label="Compare-at price">
            <input
              ...
              onChange={(e) =>
                update("compare_price", e.target.value ? Number(e.target.value) : null)
              }
              className="inp"
            />
          </Field>
            ...
            onClick={() =>
              update("images", [
                ...form.images,
                {
                  ...
                  display_order: form.images.length,
                },
              ])
            }
      ...
      >
        <p className="text-xs text-muted-foreground -mt-2 mb-3">
          Upload files to Supabase Storage or paste image URLs directly.
        </p>
              ...
              <div className="flex-1 flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    update(
                      "images",
                      form.images.map((x, idx) =>
                        idx === i ? { ...x, image_url: e.target.value } : x,
                      ),
                    )
                  }
                  placeholder="https://…"
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        update(
                          "images",
                          form.images.map((x, idx) =>
                            idx === i ? { ...x, image_url: data.publicUrl } : x,
                          ),
                        );
              ...
              <input
                ...
                onChange={(e) =>
                  update(
                    "images",
                    form.images.map((x, idx) =>
                      idx === i ? { ...x, alt_text: e.target.value } : x,
                    ),
                  )
                }
                placeholder="alt"
              ...
              <label className="text-xs flex items-center gap-1">
                <input
                  ...
                  onChange={() =>
                    update(
                      "images",
                      form.images.map((x, idx) => ({ ...x, is_primary: idx === i })),
                    )
                  }
                />{" "}
                Primary
        ...
        action={
          <button
            onClick={() =>
              update("variants", [
                ...form.variants,
                { size: "", color: "", stock_quantity: 0, price_modifier: 0 },
              ])
            }
            className="text-xs underline flex items-center gap-1"
          >
            ...
            <div key={i} className="flex gap-2 items-center">
              <input
                ...
                onChange={(e) =>
                  update(
                    "variants",
                    form.variants.map((x, idx) => (idx === i ? { ...x, size: e.target.value } : x)),
                  )
                }
                className="inp w-24"
              />
              ...
              <input
                ...
                onChange={(e) =>
                  update(
                    "variants",
                    form.variants.map((x, idx) =>
                      idx === i ? { ...x, stock_quantity: Number(e.target.value) } : x,
                    ),
                  )
                }
                className="inp w-24"
              ...
              <input
                ...
                onChange={(e) =>
                  update(
                    "variants",
                    form.variants.map((x, idx) =>
                      idx === i ? { ...x, price_modifier: Number(e.target.value) } : x,
                    ),
                  )
                }
                className="inp w-28"
      ...
      <div className="flex gap-3 sticky bottom-0 bg-secondary py-4 -mx-8 px-8 border-t border-border">
        ...
        <button
          onClick={save}
          disabled={busy}
          className="bg-primary text-primary-foreground px-6 py-2 text-sm uppercase tracking-wider disabled:opacity-50"
        >
